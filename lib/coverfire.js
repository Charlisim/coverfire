...
Usage: coverfire [options]
  ...
  --out <dir>            Write report.json, report.md, and index.html (Actions: coverfire-report)
...
export function bar(pctValue, width = 10) {
  const clamped = Math.max(0, Math.min(100, Number(pctValue) || 0));
  const filled = Math.round((clamped / 100) * width);
  return `${"█".repeat(filled)}${"░".repeat(width - filled)}`;
}

export function fileStats(coverage) {
  return [...coverage.entries()]
    .map(([filePath, hits]) => {
      const t = totals(new Map([[filePath, hits]]));
      return { path: filePath, covered: t.covered, total: t.total, pct: t.pct };
    })
    .sort((a, b) => a.pct - b.pct || a.path.localeCompare(b.path));
}

export function blobLink(github, file, start, end) {
  if (!github?.owner || !github?.repo || !github?.sha || github.sha === "HEAD") return null;
  const base = github.server || "https://github.com";
  const frag = start === end ? `L${start}` : `L${start}-L${end}`;
  return `${base}/${github.owner}/${github.repo}/blob/${github.sha}/${encodeURI(file)}#${frag}`;
}

function missLabel(u) {
  return u.start === u.end ? `${u.file}:${u.start}` : `${u.file}:${u.start}-${u.end}`;
}

export function renderMarkdown(result) {
  const { project, patch, verdict, component, byFile = [], github } = result;
  const title = component ? `coverfire · ${component}` : "coverfire";
  const stamp = verdict.ok ? "CLEAR" : "HOLD FIRE";
  const warns = verdict.failures.map((f) => `> **Warning** ${f}`).join("\n\n");
  const worst = byFile.filter((f) => f.total).slice(0, 12);
  const worstBlock = worst.length
    ? `
<details><summary>Worst files</summary>\n\n\`\`\`\n${worst
        .map((f) => `${bar(f.pct)}  ${pct(f.pct).toString().padStart(6)}%  ${f.path}`)
        .join("\n")}\n\`\`\`\n\n</details>`
    : "";
  const missed = patch.uncovered.slice(0, 40);
  const missBlock = missed.length
    ? `\n### Uncovered in this diff\n\n${missed
        .map((u) => {
          const label = missLabel(u);
          const href = blobLink(github, u.file, u.start, u.end);
          return href ? `- [\`${label}\`](${href})` : `- \`${label}\``;
        })
        .join("\n")}${patch.uncovered.length > 40 ? `\n- …${patch.uncovered.length - 40} more` : ""}`
    : "";
  return [
    MARKER,
    `## ${title}`,
    "",
    `**${pct(project.pct)}%** project · **${patch.total ? `${pct(patch.pct)}%` : "n/a"}** patch · **${stamp}**`,
    "",
    warns,
    "```",
    `project  ${bar(project.pct)}  ${pct(project.pct)}%  ${project.covered}/${project.total}  ${fmtDelta(verdict.delta)}`,
    `patch    ${patch.total ? `${bar(patch.pct)}  ${pct(patch.pct)}%  ${patch.covered}/${patch.total}` : "no executable lines in diff"}`,
    "```",
    worstBlock,
    missBlock,
    "",
    `<sub>Posted by coverfire. Open the <code>coverfire-report</code> artifact for the full HTML.</sub>`,
  ]
    .filter((x) => x != null && x !== "")
    .join("\n")
    .replace(/\n{3,}/g, "\n\n");
...
export function renderHtml(result) {
  const { project, patch, verdict, component, byFile = [] } = result;
  const stamp = verdict.ok ? "CLEAR" : "HOLD FIRE";
  const files = byFile.filter((f) => f.total);
  const miss = patch.uncovered
    .slice(0, 80)
    .map((u) => {
      const href = blobLink(result.github, u.file, u.start, u.end);
      const label = esc(missLabel(u));
  ...
  const rows = files
    .map(
      (f) =>
        `<tr><td class="bar">${bar(f.pct)}</td><td class="n">${pct(f.pct)}%</td><td>${esc(f.path)}</td></tr>`,
    )
    .join("");
  return `<!DOCTYPE html>
<html lang="en">
...
<title>coverfire${component ? ` · ${esc(component)}` : ""}</title>
<style>
  :root { --bg:#12100c; --paper:#efe6d2; --muted:#9c9486; --tracer:#ff4b12; --ok:#8adf3e; }
  html,body { margin:0; background:var(--bg); color:var(--paper); font:14px/1.5 ui-monospace, SFMono-Regular, Menlo, monospace; }
  main { width:min(880px, calc(100% - 40px)); margin:32px auto 64px; }
  h1 { font:800 42px/0.9 ui-sans-serif, system-ui; letter-spacing:-0.04em; text-transform:uppercase; margin:0 0 8px; }
  .stamp { display:inline-block; border:2px solid var(--tracer); color:var(--tracer); padding:2px 8px; font-weight:700; letter-spacing:0.12em; }
  .stamp.ok { border-color:var(--ok); color:var(--ok); }
  .lede { color:var(--muted); }
  .nums { font-variant-numeric: tabular-nums; white-space:pre; margin:22px 0; }
  table { width:100%; border-collapse:collapse; font-variant-numeric:tabular-nums; }
  td { padding:6px 8px 6px 0; border-bottom:1px solid rgba(239,230,210,.12); vertical-align:top; }
  td.bar { color:var(--tracer); width:12ch; }
  td.n { width:5ch; }
  a { color:var(--tracer); }
...
<body><main>
  <h1>coverfire</h1>
  <p><span class="stamp${verdict.ok ? " ok" : ""}">${stamp}</span></p>
  <p class="lede">${pct(project.pct)}% project · ${patch.total ? `${pct(patch.pct)}%` : "n/a"} patch · ${fmtDelta(verdict.delta)} vs base</p>
  <pre class="nums">project  ${bar(project.pct)}  ${pct(project.pct)}%  ${project.covered}/${project.total}
patch    ${patch.total ? `${bar(patch.pct)}  ${pct(patch.pct)}%  ${patch.covered}/${patch.total}` : "n/a"}</pre>
  ${verdict.failures.length ? `<p>${verdict.failures.map(esc).join("<br>")}</p>` : ""}
  <h2>Files</h2>
  <table>${rows}</table>
  ${miss ? `<h2>Uncovered in this diff</h2><ul>${miss}</ul>` : ""}
</main></body></html>
`;
}
...
export function writeReport(dir, result) {
  ...
  fs.writeFileSync(path.join(dir, "report.md"), renderMarkdown(result));
  fs.writeFileSync(path.join(dir, "index.html"), renderHtml(result));
}
...
export async function run(argv, env = process.env, io = {}) {
  ...
  const outDir = args.out ?? fileCfg.out ?? (inActions ? "coverfire-report" : null);
  if (outDir) {
    writeReport(path.resolve(cwd, outDir), result);
    if (!args.quiet) log(`Wrote report to ${outDir}/`);
  }
